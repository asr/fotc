...
module NonSchemas where
  ...
  postulate f₁-refl : (x : D) → f₁ x ≡ f₁ x
  {-# ATP prove f₁-refl #-}

  postulate f₂-refl : (x y : D) → f₂ x y ≡ f₂ x y
  {-# ATP prove f₂-refl #-}

  postulate f₃-refl : (x y z : D) → f₃ x y z ≡ f₃ x y z
  {-# ATP prove f₃-refl #-}

  postulate id : P → P
  {-# ATP prove id #-}

  postulate id₁ : {x : D} → P₁ x → P₁ x
  {-# ATP prove id₁ #-}

  postulate id₂ : {x y : D} → P₂ x y → P₂ x y
  {-# ATP prove id₂ #-}

  postulate ∨-comm : P ∨ Q → Q ∨ P
  {-# ATP prove ∨-comm #-}

  postulate ∨-comm₂ : {x y : D} → P₂ x y ∨ Q₂ x y → Q₂ x y ∨ P₂ x y
  {-# ATP prove ∨-comm₂ #-}
...
module Schemas where
  ...
  postulate f₃-refl : (f₃ : D → D → D → D)(x y z : D) → f₃ x y z ≡ f₃ x y z
  {-# ATP prove f₃-refl #-}
  ...
  postulate id₂ : {P₂ : D → D → Set}{x y : D} → P₂ x y → P₂ x y
  {-# ATP prove id₂ #-}
...
module SchemeInstances where
  ...
  postulate scheme : (P Q : D → Set){x : D} → P x → Q x
