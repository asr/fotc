...
0<0→⊥ : ¬ (zero < zero)
0<0→⊥ 0<0 = true≢false (trans (sym 0<0) lt-00)

S<0→⊥ : ∀ {n} → ¬ (succ₁ n < zero)
S<0→⊥ {n} h = true≢false (trans (sym h) (lt-S0 n))
...
0>S→⊥ : ∀ {n} → ¬ (zero > succ₁ n)
0>S→⊥ {n} h = true≢false (trans (sym h) (lt-S0 n))
...
S≤0→⊥ nzero S0≤0 = true≢false (trans (sym S0≤0) (trans (lt-SS zero zero) lt-00))
S≤0→⊥ (nsucc {n} _) SSn≤0 =
  true≢false (trans (sym SSn≤0) (trans (lt-SS (succ₁ n) zero) (lt-S0 n)))
...
S≯0→⊥ : ∀ {n} → ¬ (succ₁ n ≯ zero)
S≯0→⊥ {n} h = ⊥-elim (true≢false (trans (sym (lt-0S n)) h))
