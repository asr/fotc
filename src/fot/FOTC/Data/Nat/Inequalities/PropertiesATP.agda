...
x≮y→x≥y nzero (nsucc {n} Nn) 0≮Sn =
  ⊥-elim (true≢false (trans (sym (<-0S n)) 0≮Sn))
...
pred-≤ nzero = prf
  where postulate prf : pred₁ zero ≤ zero ≡ true
        {-# ATP prove prf #-}

pred-≤ (nsucc {n} Nn) = prf
  where postulate prf : pred₁ (succ₁ n) ≤ succ₁ n ≡ true
...
postulate x∸Sy≤x∸y : ∀ {m n} → N m → N n → LE (m ∸ succ₁ n) (m ∸ n)
