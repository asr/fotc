...
module Interactive.FOTC.Data.Nat.Inequalities.Properties where
...
-- Interactive.FOTC.Data.Nat.Inequalities.EliminationProperties.
...
-- TODO (2019-06-09): Missing proof.
postulate
  x+k<y+k→x<y : ∀ {m n k} → N m → N n → N k → m + k < n + k → m < n
...
x≯Sy→x≯y∨x≡Sy (nsucc {m} Nm) (nsucc {n} Nn) h = prf (x≯Sy→x≯y∨x≡Sy Nm Nn m≯Sn)
  ...
  postulate
    m≯Sn : m ≯ succ₁ n
    prf : m ≯ n ∨ m ≡ succ₁ n → succ₁ m ≯ succ₁ n ∨ succ₁ m ≡ succ₁ (succ₁ n)
