...
module Draft.FO-LTC-PCF.Program.Division.EquationsI where
...
-- λ-abstractions, but we can use it with the version of div using
-- super-combinators.
...
private
    ...
    -- From @div · i · j@ to @div-s₁@ using the conversion rule
    -- @fix-eq@.
    ...
    proof₁₋₂ i j =
      subst (λ t → t · j ≡ fun i · j)
            ...
            refl
         where
         -- The function @fun@ is the same that the @fun@ part of
         -- @div-s₂@, except that we need a fresh variable @y@ to
         -- avoid the clashing of the variable @i@ in the application
         -- of the @beta@ rule.
         fun : D → D
